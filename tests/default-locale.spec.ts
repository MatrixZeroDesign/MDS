...
test("new visits and example code default to English", async ({ page }) => {
	...
	await expect(page.locator(".docs-reference-detail pre")).toContainText('locale = "en"');
	await expect(page.locator("html")).toHaveAttribute("lang", "en");
	await page.getByRole("button", { name: "Change language" }).click();
	await expect(
		page.getByRole("region", { name: "交互示例" }).getByRole("button", { name: "添加", exact: true }),
	).toBeVisible();
	await expect(page.locator(".docs-reference-detail pre")).toContainText('locale = "zh"');
});
