...
const colorOf = (series: { color?: string }, index: number) => series.color ?? colors[index % colors.length];
...
function CartesianChart({
	...
	formatAxisValue,
	stacked = false,
}: CartesianChartProps & { kind: "line" | "area" | "bar" }) {
	...
	const dot = { r: 3.5, stroke: "var(--mds-surface)", strokeWidth: 2 };
	// Straight segments express observed values without implying a smoothed measurement.
	const chart =
		kind === "line" ? (
			<RLineChart {...common}>
				{axes}
				{series.map((s, i) => (
					<Line
						...
						stroke={colorOf(s, i)}
						strokeWidth={2}
						dot={sanitized.length === 1 ? dot : false}
						activeDot={dot}
						connectNulls={false}
						isAnimationActive={false}
					/>
...
export const LineChart = (props: CartesianChartProps) => <CartesianChart {...props} kind="line" />;
export const AreaChart = (props: CartesianChartProps) => <CartesianChart {...props} kind="area" />;
export const BarChart = (props: CartesianChartProps) => <CartesianChart {...props} kind="bar" />;
