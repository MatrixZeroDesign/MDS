...
test("showcase catalog filters twelve scenarios and clears empty search", async ({ page }) => {
	await page.goto("/#showcase");
	await expect(page.locator(".sc-gallery-card")).toHaveCount(12);
	await page.getByRole("radio", { name: "Consumer · 5", exact: true }).click();
	await expect(page.locator(".sc-gallery-card")).toHaveCount(5);
